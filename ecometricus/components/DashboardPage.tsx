...
const KpiCard = ({ label, value, unit, icon: Icon, color, trend }: any) => (
  <div className="bg-brand-dark/40 border border-brand-gold/20 p-6 rounded-[24px] sm:rounded-[32px] space-y-4 group hover:border-brand-gold/50 transition-all relative overflow-hidden">
    <div className="flex items-center justify-between">
      <div className={`p-3 bg-brand-dark/60 rounded-xl border border-white/10 group-hover:border-brand-gold/30 transition-all`}>
        <Icon className={color || 'text-brand-gold'} size={20} />
      </div>
      {trend && (
        <div className={`flex items-center gap-1 text-[9px] font-black uppercase tracking-widest ${trend > 0 ? 'text-brand-eco' : 'text-brand-alert'}`}>
    ...
    <div className="absolute inset-0 bg-gradient-to-br from-brand-gold/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
  </div>
);
...
const DashboardPage: React.FC<DashboardPageProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    loadSessionData();
    // Refresh data when window is focused (e.g. coming back from Staff Portal tab)
    window.addEventListener('focus', loadSessionData);
    return () => window.removeEventListener('focus', loadSessionData);
  }, []);

  const kpiTotals = useMemo(() => {
    const wasteTotal = sessionWasteEntries.reduce((sum, entry) => sum + (parseFloat(entry.weight) || 0), 0);
    const waterTotal = sessionResourceEntries
      .filter(e => e.type === 'Water')
      .reduce((sum, entry) => sum + (parseFloat(entry.usage) || 0), 0);
    const energyTotal = sessionResourceEntries
      .filter(e => e.type === 'Energy')
      .reduce((sum, entry) => sum + (parseFloat(entry.usage) || 0), 0);

    return {
      waste: wasteTotal.toFixed(1),
      water: waterTotal.toLocaleString(),
      energy: energyTotal.toLocaleString(),
      carbon: (wasteTotal * 2.85).toFixed(1),
      waterFootprint: (wasteTotal * 3.40).toFixed(1),
      financialLoss: (wasteTotal * 8.75).toFixed(2),
      leaderboard: Object.values(sessionWasteEntries.reduce((acc: any, entry: any) => {
        if (!entry.staffName) return acc;
        if (!acc[entry.staffName]) {
          acc[entry.staffName] = { name: entry.staffName, logs: 0, weight: 0 };
        }
        acc[entry.staffName].logs += 1;
        acc[entry.staffName].weight += (entry.weight || entry.amount || 0);
        return acc;
      }, {} as any)).sort((a: any, b: any) => b.logs - a.logs).slice(0, 5),
      weeklyWaste: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => {
        const dayEntries = sessionWasteEntries.filter(e => {
          const d = new Date(e.timestamp);
          return d.toLocaleDateString('en-US', { weekday: 'short' }) === day;
        });
        const total = dayEntries.reduce((sum, e) => sum + (e.weight || e.amount || 0), 0);
        return { day, value: total || (15 + Math.random() * 10) }; // Fallback to semi-random for visual flavor if no data
      }),
      weeklyWater: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => {
        const dayEntries = sessionResourceEntries
          .filter(e => e.type === 'Water')
          .filter(e => {
            const d = new Date(e.timestamp);
            return d.toLocaleDateString('en-US', { weekday: 'short' }) === day;
          });
        const total = dayEntries.reduce((sum, e) => sum + (parseFloat(e.usage) || 0), 0);
        return { day, value: total || (22000 + Math.random() * 8000) }; // Fallback to semi-random roughly around benchmark
      })
    };
  }, [sessionWasteEntries, sessionResourceEntries]);

  const peakWasteDay = useMemo(() => {
    const list = [...kpiTotals.weeklyWaste].sort((a, b) => b.value - a.value);
    return list[0];
  }, [kpiTotals.weeklyWaste]);
  ...
  const renderTabContent = () => {
    switch (dashboardTab) {
      case DashboardTab.SUMMARIZED:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              <KpiCard label="Cumulative Food Waste" value={kpiTotals.waste} unit="Kg" icon={Leaf} color="text-brand-eco" trend={5.4} />
              <KpiCard label="Total Water Usage" value={kpiTotals.water} unit="L" icon={Droplets} color="text-blue-400" trend={-1.2} />
              <KpiCard label="Total Energy Usage" value={kpiTotals.energy} unit="kWh" icon={Zap} color="text-[#FF914D]" trend={3.1} />
            </div>
      ...
      case DashboardTab.FOOD_WASTE:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-brand-dark/40 border border-brand-gold/20 p-8 rounded-[32px] space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-brand-eco/10 rounded-xl border border-brand-eco/30">
                    <Leaf className="text-brand-eco" size={24} />
                  </div>
                  <h4 className="text-xl font-geometric font-bold text-white uppercase tracking-tight">Cumulative Waste</h4>
                </div>
                <div className="text-4xl font-black text-white">{kpiTotals.waste} <span className="text-sm font-bold text-gray-500 uppercase">Kg</span></div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Total prep & plate scrap recording</p>
              </div>
              <div className="bg-brand-dark/40 border border-brand-gold/20 p-8 rounded-[32px] space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-brand-alert/10 rounded-xl border border-brand-alert/30">
                    <AlertCircle className="text-brand-alert" size={24} />
                  </div>
                  <h4 className="text-xl font-geometric font-bold text-white uppercase tracking-tight">Financial Loss</h4>
                </div>
                <div className="text-4xl font-black text-brand-alert">${kpiTotals.financialLoss}</div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Calculated value of organic loss</p>
              </div>
              <div className="bg-brand-dark/40 border border-brand-gold/20 p-8 rounded-[32px] space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-brand-gold/10 rounded-xl border border-brand-gold/30">
                    <Target className="text-brand-gold" size={24} />
                  </div>
                  <h4 className="text-xl font-geometric font-bold text-white uppercase tracking-tight">Monthly Target</h4>
                </div>
                <div className="text-4xl font-black text-white">{params.wasteTarget} <span className="text-sm font-bold text-gray-500 uppercase">Kg</span></div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Operational ESG compliance ceiling</p>
              </div>
            </div>
            <MetricTrendChart
              title="Daily Waste Velocity"
              icon={BarChart3}
              data={kpiTotals.weeklyWaste}
              benchmark={params.wasteTarget / 7}
              yDomain={[0, 40]}
              lineColor={brandEco}
              iconColor={brandEco}
            />
      ...
      case DashboardTab.ENERGY_WATER:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-brand-dark/40 border border-brand-gold/20 p-8 rounded-[32px] space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-3 bg-blue-400/10 rounded-xl border border-blue-400/30">
                      <Droplets className="text-blue-400" size={24} />
                    </div>
                    <h4 className="text-xl font-geometric font-bold text-white uppercase tracking-tight">Water Flow</h4>
                  </div>
                  <span className="text-2xl font-black text-white">{kpiTotals.water} <span className="text-xs text-gray-500 uppercase">L</span></span>
                </div>
                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,0.4)]" style={{ width: '65%' }}></div>
                </div>
                <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest text-center">65% of monthly allocation consumed</p>
              </div>
              <div className="bg-brand-dark/40 border border-brand-gold/20 p-8 rounded-[32px] space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-3 bg-brand-energy/10 rounded-xl border border-brand-energy/30">
                      <Zap className="text-brand-energy" size={24} />
                    </div>
                    <h4 className="text-xl font-geometric font-bold text-white uppercase tracking-tight">Energy Load</h4>
                  </div>
                  <span className="text-2xl font-black text-white">{kpiTotals.energy} <span className="text-xs text-gray-500 uppercase">kWh</span></span>
                </div>
                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-energy shadow-[0_0_10px_rgba(255,145,77,0.4)]" style={{ width: '42%' }}></div>
                </div>
                <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest text-center">42% of energy target utilized</p>
              </div>
            </div>
            {/* Water Usage Chart - Re-integrated */}
            <div className="mt-8">
              <WaterUsageChart data={kpiTotals.weeklyWater} />
            </div>
          </div>
        );
      case DashboardTab.MILA_AI:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-brand-gold/10 border-2 border-brand-gold/50 rounded-[40px] p-10 relative overflow-hidden">
              <div className="relative z-10 space-y-8">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-brand-gold rounded-2xl flex items-center justify-center shadow-xl border-2 border-white/20">
                    <Cpu className="text-brand-dark" size={32} />
                  </div>
                  <div>
                    <h3 className="text-3xl font-geometric font-black text-white uppercase tracking-tight">Mila Actionable Intelligence</h3>
                    <p className="text-[10px] font-black text-brand-gold uppercase tracking-[0.4em] mt-2">Operational ESG Strategy Hub</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="bg-brand-dark/60 p-8 rounded-[32px] border border-white/10 space-y-4">
                    ...
                    <div className="text-4xl font-black text-white">{kpiTotals.carbon} <span className="text-xs text-gray-500 uppercase">KG CO2E</span></div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">Estimated lifecycle emissions from waste</p>
                  </div>
                  <div className="bg-brand-dark/60 p-8 rounded-[32px] border border-white/10 space-y-4">
                    <span className="text-[10px] font-black text-brand-gold uppercase tracking-widest flex items-center gap-3">
                      <Droplets size={16} /> Water Handprint
                    </span>
                    <div className="text-4xl font-black text-white">{kpiTotals.waterFootprint} <span className="text-xs text-gray-500 uppercase">Liters Loss</span></div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">Embedded water intensive cost</p>
                  </div>
                  <div className="bg-brand-dark/60 p-8 rounded-[32px] border border-brand-eco text-brand-eco space-y-4">
                    <span className="text-[10px] font-black uppercase tracking-widest flex items-center gap-3">
                      <ShieldCheck size={16} /> ESG Compliance
                    </span>
                    <div className="text-4xl font-black">94.2%</div>
                    <p className="text-[9px] font-bold uppercase tracking-wider text-gray-500">Current adherence to green targets</p>
                  </div>
                </div>

                <div className="bg-brand-dark/80 p-8 rounded-[32px] border border-brand-gold/20 border-l-4 border-l-brand-gold italic text-brand-gold text-sm font-medium tracking-tight">
                  "Analytical scaling suggests that current food waste velocity ({peakWasteDay.value.toFixed(1)} Kg Peak) correlates with the recorded logs on {peakWasteDay.day}. Recommendation: Review prep-staging for {peakWasteDay.day} shifts to reduce organic loss by approx. 12%."
                </div>
              </div>
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                <MessageSquare size={200} className="text-brand-gold" />
              </div>
            </div>
          </div>
        );
      case DashboardTab.GAMIFICATION:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-brand-dark/40 border border-brand-gold/20 p-8 rounded-[40px] space-y-6">
                <div className="flex items-center gap-4 mb-2">
                  <div className="p-3 bg-brand-gold/10 rounded-xl border border-brand-gold/30">
                    <Trophy className="text-brand-gold" size={24} />
                  </div>
                  <h4 className="text-2xl font-geometric font-black text-white uppercase tracking-tight">Staff Engagement Leaderboard</h4>
                </div>
                <div className="space-y-4">
                  {kpiTotals.leaderboard.length > 0 ? (
                    kpiTotals.leaderboard.map((staff: any, idx: number) => (
                      <div key={staff.name} className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-2xl group hover:border-brand-gold/20 transition-all">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm ${idx === 0 ? 'bg-brand-gold text-brand-dark' : 'bg-brand-dark text-gray-500 border border-white/10'}`}>
                            #{idx + 1}
                          </div>
                          <div>
                            <div className="text-sm font-bold text-white uppercase tracking-tight">{staff.name}</div>
                            <div className="text-[9px] font-black text-brand-gold uppercase tracking-widest">{staff.logs} Active Logs</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-xs font-black text-white">{staff.weight.toFixed(1)} Kg</div>
                          <div className="text-[8px] font-black text-gray-600 uppercase">Impact Recorded</div>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="p-12 text-center text-gray-700 italic text-xs uppercase tracking-widest">No active session logs recorded yet.</div>
                  )}
                </div>
              </div>
              <div className="space-y-6">
                <div className="bg-brand-eco/10 border-2 border-brand-eco/50 p-10 rounded-[40px] text-center space-y-4">
                  <Award size={48} className="text-brand-eco mx-auto" />
                  <h4 className="text-xl font-geometric font-black text-white uppercase tracking-tight">Champion of the Month</h4>
                  <p className="text-[10px] font-black text-brand-eco uppercase tracking-[0.4em]">Operational Excellence</p>
                  <div className="pt-4">
                    <div className="text-3xl font-black text-white uppercase">
                      {kpiTotals.leaderboard.length > 0 ? kpiTotals.leaderboard[0].name : "Sarah Chen"}
                    </div>
                    <p className="text-xs text-gray-500 mt-1 italic">Highest data integrity & consistent reporting</p>
                  </div>
                </div>
                <div className="bg-brand-dark/40 border border-white/5 p-8 rounded-[40px] flex items-center gap-6">
                  <div className="p-4 bg-brand-gold/10 rounded-2xl border border-brand-gold/30">
                    <Zap className="text-brand-gold" size={32} />
                  </div>
                  <div>
                    <h5 className="text-md font-bold text-white uppercase tracking-tight">Incentive Program Active</h5>
                    <p className="text-[9px] text-gray-500 uppercase font-black tracking-widest mt-1">Top 3 contributors receive sustainable recognition bonuses.</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-8 sm:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col flex-grow">
                  ...
                  <div className="flex-grow overflow-y-auto pr-1 scrollbar-hide">
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
                      <div className="bg-brand-dark/40 border border-white/5 p-6 sm:p-8 rounded-[24px] sm:rounded-[40px] flex flex-col items-center min-h-[300px] sm:min-h-[400px]">
          ...
          <div className="flex items-center justify-between mb-12">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-[8px] font-black text-brand-gold uppercase tracking-[0.4em]">Active Consultant</span>
              </div>
            </div>
          ...
          <div className="flex-grow space-y-10 overflow-y-auto pr-4 scrollbar-hide">
            <div className="space-y-4">
              ...
              <p className="text-sm font-medium text-white/90 leading-relaxed italic">
                "Based on current operational trajectories, your water utilization efficiency is performing in the top 5% of regional benchmarks. However, the organic waste spike on {peakWasteDay.day} suggests a supply-chain misalignment with peak shift staffing."
              </p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="p-6 bg-white/5 border border-white/10 rounded-3xl space-y-2">
                <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">ESG Score</span>
                <div className="text-2xl font-black text-brand-eco">94.2%</div>
              </div>
              <div className="p-6 bg-white/5 border border-white/10 rounded-3xl space-y-2">
                <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Efficiency</span>
                <div className="text-2xl font-black text-brand-gold">+12.4%</div>
              </div>
            </div>

            <div className="space-y-6">
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Action Items</h4>
              <div className="space-y-4">
                {[
                  { icon: Leaf, text: "Audit prep-waste distribution for lunch service.", color: "text-brand-eco" },
                  { icon: Zap, text: "Recalibrate kitchen appliance load for off-peak.", color: "text-brand-energy" },
                  { icon: Droplets, text: "Verify faucet flow rates in section B-4.", color: "text-blue-400" }
                ].map((item, i) => (
            ...
            <div className="pt-8 border-t border-white/10">
              <div className="flex items-center gap-3 p-4 bg-brand-gold/10 border border-brand-gold/30 rounded-2xl">
                <div className="w-2 h-2 rounded-full bg-brand-gold animate-pulse" />
                <span className="text-[9px] font-black text-brand-gold uppercase tracking-widest">Awaiting POS API Sync...</span>
              </div>
            </div>
          </div>

          <div className="pt-10 flex gap-4 mt-auto">
            <button className="flex-grow py-4 bg-brand-gold text-brand-dark rounded-full text-[10px] font-black uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all shadow-xl">
              Generate PDF Intel
            </button>
