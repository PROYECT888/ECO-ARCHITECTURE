...
const StaffPortal: React.FC<StaffPortalProps> = ({ user, onLogout }) => {
  ...
  const [lastImpactEntry, setLastImpactEntry] = useState<{ carbon: number, water: number, cost: number } | null>(null);
  ...
  const handleSaveWaste = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      // Calculate specific impact for this log
      const carbon = newEntry.amount * 2.85;
      const water = newEntry.amount * 3.40;
      const cost = (newEntry.amount * 7.50) + (newEntry.amount * 1.25);
      setLastImpactEntry({ carbon, water, cost });
      setShowImpactToast(true);
      setTimeout(() => setShowImpactToast(false), 5000);
  ...
  return (
    ...
    >
      ...
      {/* Real-Time Impact Feedback Toast */}
      {showImpactToast && lastImpactEntry && (
        <div className="fixed top-24 sm:top-28 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md animate-in slide-in-from-top-4 duration-500">
          <div className="bg-[#0f2420]/95 backdrop-blur-2xl border-2 border-brand-eco rounded-[30px] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col items-center text-center space-y-4">
            <div className="w-14 h-14 bg-brand-eco rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(119,177,57,0.4)] border-2 border-white/20">
              <CheckCircle2 size={32} className="text-brand-dark" />
            </div>
            <div>
              <h4 className="text-lg font-black text-white uppercase tracking-tight">Entry Impact Verified</h4>
              <p className="text-[9px] font-black text-brand-eco uppercase tracking-[0.3em] mt-1">Sustainability Contribution Logged</p>
            </div>
            <div className="grid grid-cols-3 gap-4 w-full pt-2">
              <div className="flex flex-col items-center">
                <Cloud size={14} className="text-gray-500 mb-1" />
                <span className="text-xs font-black text-white">{lastImpactEntry.carbon.toFixed(1)}kg</span>
                <span className="text-[7px] font-black text-gray-500 uppercase">CO2e</span>
              </div>
              <div className="flex flex-col items-center border-x border-white/10">
                <Droplets size={14} className="text-blue-500 mb-1" />
                <span className="text-xs font-black text-white">{lastImpactEntry.water.toFixed(1)}L</span>
                <span className="text-[7px] font-black text-gray-500 uppercase">Water</span>
              </div>
              <div className="flex flex-col items-center">
                <DollarSign size={14} className="text-brand-gold mb-1" />
                <span className="text-xs font-black text-white">${lastImpactEntry.cost.toFixed(1)}</span>
                <span className="text-[7px] font-black text-gray-500 uppercase">Financial</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-grow p-4 sm:p-8 lg:p-14 xl:p-20 space-y-8 sm:space-y-12 lg:space-y-16">
        {/* Experience Header: Personal Impact & Shift Summary */}
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 overflow-visible">
          <div className="md:col-span-2 bg-gradient-to-br from-[#122b26] to-[#0a1815] border-2 border-white/10 rounded-[40px] p-8 relative overflow-hidden group shadow-2xl">
            <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-20 transition-all pointer-events-none">
              <TrendingUp size={140} className="text-brand-eco" />
            </div>
            <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              <div className="space-y-2">
                <span className="text-[9px] font-black text-brand-eco uppercase tracking-[0.4em]">Shift Achievement — {user.position}</span>
                <h2 className="text-2xl sm:text-4xl font-black text-white uppercase tracking-tight leading-none">Your Impact Today</h2>
              </div>
              <div className="flex gap-4">
                <div className="flex flex-col items-end">
                  <div className="text-2xl font-black text-brand-gold">{wasteEntries.length}</div>
                  <span className="text-[8px] font-black text-gray-500 uppercase">Logs Verified</span>
                </div>
                <div className="w-[1px] h-10 bg-white/10" />
                <div className="flex flex-col items-end">
                  <div className="text-2xl font-black text-brand-eco">{(wasteEntries.length * 15).toFixed(0)}</div>
                  <span className="text-[8px] font-black text-gray-500 uppercase">ESG Points</span>
                </div>
              </div>
            </div>
          </div>

          <div className="bg-brand-dark/60 border-2 border-brand-gold/20 rounded-[40px] p-8 flex flex-col justify-center relative overflow-hidden group hover:border-brand-gold/40 transition-all cursor-crosshair">
            <div className="flex items-center gap-4 mb-4">
              <div className="p-3 bg-brand-gold/10 rounded-2xl border-2 border-brand-gold/30">
                <Star className="text-brand-gold" size={20} />
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-white uppercase tracking-widest">Efficiency Status</span>
                <span className="text-[8px] font-black text-brand-gold uppercase tracking-[0.2em] mt-0.5">Shift Benchmark Active</span>
              </div>
            </div>
            <div className="w-full bg-white/5 h-2.5 rounded-full overflow-hidden border border-white/5 relative shadow-inner">
              <div
                className={`h-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(119,177,57,0.5)] ${totals.waste > BENCHMARKS.waste ? 'bg-brand-alert' : 'bg-brand-eco'}`}
                style={{ width: `${Math.min(100, (totals.waste / BENCHMARKS.waste) * 100)}%` }}
              />
              <div className="absolute top-0 right-10 h-full w-[2px] bg-brand-gold/40 z-10" />
            </div>
            <div className="flex justify-between mt-3">
              <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Current usage</span>
              <span className={`text-[10px] font-black uppercase ${totals.waste > BENCHMARKS.waste ? 'text-brand-alert' : 'text-brand-eco'}`}>
                {((totals.waste / BENCHMARKS.waste) * 100).toFixed(0)}%
              </span>
            </div>
          </div>
        </div>

        {/* Mila Lite Frontline Guidance */}
        <div className="max-w-6xl mx-auto py-4 bg-brand-gold/5 border-y-2 border-brand-gold/20 backdrop-blur-md overflow-hidden flex items-center gap-6 group hover:bg-brand-gold/10 transition-all">
          <div className="px-6 border-r-2 border-brand-gold/20 flex items-center gap-3 shrink-0">
            <div className="w-8 h-8 bg-brand-gold rounded-lg flex items-center justify-center shadow-lg">
              <Cpu size={16} className="text-brand-dark" />
            </div>
            <span className="text-[10px] font-black text-white uppercase tracking-widest">Mila Lite</span>
          </div>
          <div className="flex-grow overflow-hidden relative h-6">
            <div className="absolute inset-0 flex items-center animate-scroll-text whitespace-nowrap">
              <span className="text-[10px] font-black text-brand-gold uppercase tracking-[0.2em] px-8">• Tip: Proper segregation of Prep Waste can reduce financial loss by 12% •</span>
              <span className="text-[10px] font-black text-white uppercase tracking-[0.2em] px-8">• You've logged {wasteEntries.length} items - precision is the key to ESG growth •</span>
              <span className="text-[10px] font-black text-brand-gold uppercase tracking-[0.2em] px-8">• Shift Target: Keep cumulative waste below {BENCHMARKS.waste}kg for bonus points •</span>
              <span className="text-[10px] font-black text-white uppercase tracking-[0.2em] px-8">• Current Peak: Station {user.outletCode} is leading in data accuracy today •</span>
            </div>
          </div>
        </div>

        {/* Daily Input Data Entry Container */}
        <div className="max-w-6xl mx-auto bg-[#0f2420] border-2 border-brand-gold/60 rounded-[30px] sm:rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.7)] relative overflow-hidden">
          <div className="p-6 sm:p-10 lg:p-12 xl:p-16">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 sm:mb-12 gap-4">
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-geometric font-black text-white uppercase tracking-tight leading-none"> Daily Input Data </h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 xl:gap-24">
                ...
                <div className="space-y-6 sm:space-y-8 lg:space-y-10">
                  <div className="space-y-4 group/field">
                    <label className="text-[10px] sm:text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] flex justify-between group-focus-within/field:text-brand-gold transition-colors">
                      Step 1: Classification <span className="text-brand-gold/40">Required</span>
                    </label>
                    <div className="relative group">
                      <select
                        ...
                        className="w-full bg-brand-dark/50 border-2 border-white/10 rounded-2xl py-4 sm:py-5 px-6 sm:px-8 text-sm sm:text-base font-bold text-white uppercase tracking-tight appearance-none focus:outline-none focus:border-brand-gold transition-all shadow-lg group-hover:border-white/20"
                      >
                        <option value="">Select Primary Classification</option>
                        {Object.keys(INVENTORY_LOGIC).map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                      <ChevronDown size={20} className="absolute right-6 top-1/2 -translate-y-1/2 text-brand-gold pointer-events-none group-hover:scale-110 transition-transform" />
                    </div>
                  </div>

                  <div className={`space-y-4 group/field transition-all duration-300 ${!form.category ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                    <label className="text-[10px] sm:text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] flex justify-between group-focus-within/field:text-brand-gold transition-colors">
                      Step 2: Sub-Category <span className="text-brand-gold/40">Required</span>
                    </label>
                    <div className="relative group">
                      <select
                        required
                  ...
                  <div className={`space-y-4 group/field transition-all duration-300 ${!form.subCategory ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                    <label className="text-[10px] sm:text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] flex justify-between group-focus-within/field:text-brand-gold transition-colors">
                      Step 3: Product <Tooltip id="prod" text="Select from inventory list or enter manually for unique items." />
                    </label>
                    <div className="relative group">
                      <input
                        ...
                        className="w-full bg-brand-dark/50 border-2 border-white/10 rounded-2xl py-4 sm:py-5 px-6 sm:px-8 text-sm sm:text-base font-bold text-white uppercase tracking-tight appearance-none focus:outline-none focus:border-brand-gold transition-all shadow-lg group-hover:border-white/20 placeholder:text-gray-700"
                      />
                      <datalist id="product-suggestions">
                        {form.category && form.subCategory && INVENTORY_LOGIC[form.category][form.subCategory].map(p => <option key={p} value={p} />)}
                      </datalist>
                      <Search size={20} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-700 pointer-events-none group-focus-within:text-brand-gold transition-colors" />
                    </div>
                  </div>

                  <div className={`space-y-4 group/field transition-all duration-300 ${!form.product ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                    <label className="text-[10px] sm:text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] flex justify-between group-focus-within/field:text-brand-gold transition-colors">
                      Step 4: Justification <Tooltip id="just" text="Explain the reason for waste (e.g., spoiled, storage issues, overproduction)." />
                    </label>
                    <div className="space-y-4">
                      ...
                      {form.justification === 'Other' && (
                        <textarea
                          ...
                          className="w-full bg-brand-dark/50 border-2 border-white/10 rounded-2xl py-4 px-6 text-sm font-bold text-white uppercase tracking-tight focus:outline-none focus:border-brand-gold transition-all h-24 lg:h-32 resize-none placeholder:text-gray-700"
                        />
                      )}
                    </div>
                  </div>
      ...
      {/* 5. Mila AI Actionable Intelligence Container */}
